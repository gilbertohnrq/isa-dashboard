...
export function GaugeMetric({
  ...
  const track = arcPath(72, 72, 46, 215, 505);
  const endAngle = 215 + progress * 290;
  const progressPath = arcPath(72, 72, 46, 215, endAngle);
  const endPoint = polarToCartesian(72, 72, 46, endAngle);
  ...
  return (
    <div className="relative flex h-[148px] w-[144px] flex-col items-center justify-end text-center">
      <svg
        viewBox="0 0 144 118"
        className="absolute left-0 top-0 h-[114px] w-[144px] overflow-visible"
        aria-hidden="true"
      >
        <path
          d={track}
          stroke="rgba(228, 235, 240, 0.28)"
          strokeWidth="9"
          fill="none"
          strokeLinecap="round"
        />
      ...
      <motion.span
        ...
        className="dashboard-number-xl z-10 mt-4"
      >
        {value}
      </motion.span>
      <span className="mt-1 text-[16px] font-[510] text-white">{label}</span>
    </div>
  );
}
